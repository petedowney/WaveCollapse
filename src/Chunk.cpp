...
Chunk::Chunk(const int x, const int y, const int size, const std::vector<float>& defaultState, const std::vector<Constraint>& constraints) {
    ...
    for (int n = 0; n < size; n++) {
        for (int n2 = 0; n2 < size; n2++) {
            this->state[n][n2] = std::make_shared<Tile>(defaultState, false);
...
/*Chunk::Chunk(int x, int y, int size, std::vector<float> defaultState, std::vector<Constraint> constraints) {
...
int Chunk::getCollapsedTileState(int x, int y) {
    return getCollapsedTileState(state[x][y]);
}

int Chunk::getCollapsedTileState(std::shared_ptr<Tile> tile) {
    for (int n = 0; n < tile->state.size(); n++) {
